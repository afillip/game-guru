...
export class GameController {
  ...
  private async _createGame(req: ICreateGameRequest, res: Response) {
    const values: ListValueFormat = [this._hydrateListValueArrayFromBoardGame(req.body)];
    ...
    try {
      const response: IAppendDataResponse = await this.googleSheetsApi.sheetsAccess.spreadsheets.values.append(
        request
      );
  ...
  private _hydrateListValueArrayFromBoardGame(boardGame: Omit<IBoardGame, 'id'>): Array<any> {
    ...
    listValue[
      GameModelToIndexRefEnum.linkToRules
    ] = `https://lmgtfy.com/?q=how+to+play+${boardGame.name.replace(' ', '')}`;
    ...
    listValue[GameModelToIndexRefEnum.firstPlaythroughDuration] =
      boardGame.firstPlaythroughDuration;
